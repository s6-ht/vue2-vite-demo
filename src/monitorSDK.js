import { replaceOld, supportsHistory } from './utils'
...
export default class Monitor {
  ...
  handleError() {
      if (this.vm) {
          this.vm.config.errorHandler = (err, vm) => {
              console.log(err)
          }
      }
    // let errorObj = {},    //错误对象
    ...
    //     console.log(e, 'sdk listen error')
        ...
        // console.log(window.location,_this.curPageUrl)
  ...
  handleRouteData(parsedFrom, parsedTo) {
    ...
    for (let i = 0; i < this.reportData.data.length - 1; i++) {
      if (
        this.reportData.data[i].page_url_to ===
        this.reportData.data[i + 1].page_url_from
      ) {
        this.reportData.data[i].exit_page_time = this.reportData.data[
          i + 1
        ].visit_page_time
      }
    }
  ...
  //vue路由重写：把原来函数存个变量，重写原来函数，把原来函数再返回出去
  replaceHistory() {
    ...
    window.onpopstate = function() {
      ...
      const to = _this.getLocationHref()
      const from = _this.curPageUrl
      _this.handleRouteData(from, to)
...
function eventFunc(e) {
  ...
  if (tagArr.includes(tagName)) {
    ...
    console.log(this.getLocationHref(), this.curPageUrl)
