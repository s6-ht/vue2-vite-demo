  ...
  try {
    switch (statement.type) {
      ...
      case "LogicalExpression":
        // 逻辑运算符 || &&
        // 只要在left中的节点都应该生成一个判断节点, right中的最后一个节点不生成判断节点
        ...
        // if (statement.left.extra) {
        //   statement.left.parentOperator = statement.operator;
        // }
        // if (statement.right.extra) {
        //   statement.right.parentOperator = statement.operator;
        // }
        // TODO: 判断节点圈复杂度统计
        ...
        if (statement.left.type === "LogicalExpression") {
          // 标记所有的left节点，用于排除最后一个节点，因为最后一个节点不用生成判断节点
          statement.left.isLeft = true;
          ({ nodes: logicalLeftNodes } = transformAstToGraph(statement.left));
        } else {
          logicalLeftNode = [
            {
              id: statement.left.mark,
              name: replaceDoubleToSingleQuotes(statement.left.name),
              shape: "square"
            }
          ];
          ...
          //   });
          // }
          // console.log(logicalLeftNode);
        }

        if (statement.right.type === "LogicalExpression") {
          if (statement.isLeft) {
            statement.right.isLeft = true;
          }
            ...
            logicalRightNode = [
              {
                ...
                name: replaceDoubleToSingleQuotes(statement.right.name),
                shape: "square"
              }
            ];
          }
